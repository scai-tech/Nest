...
def generate_alltoall_files(npus_count: int, coll_size: int, reduce_ranks: list) -> None:
    """
    Function to generate Chakra ET files for all-to-all communication.
    ...
    target_dir = os.path.join(script_dir, "tmp/network/alltoall")
    if not os.path.exists(target_dir):
        os.makedirs(target_dir)
    
    for npu_id in range(npus_count):
        output_filename = os.path.join(target_dir, f"alltoall.{npu_id}.et")
        with open(output_filename, "wb") as et:
            ...
            if npu_id in reduce_ranks:  # Use the passed list of ranks
                ...
                node.attr.extend([get_comm_type_attr(ALL_TO_ALL), ChakraAttr(name="comm_size", int64_val=coll_size)])
