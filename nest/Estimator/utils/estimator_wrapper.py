...
from wham.initialize import initialize as hardware_initializer
from wham.initialize import get_core_energy as get_external_core_energy
from wham.initialize import get_core_area as get_external_core_area
from wham.initialize import run_area_energy_generation

from wham.perf_wrappers.tensor_core_wrapper import get_dims_and_fused_perf_est as get_tc_dims
from wham.perf_wrappers.vector_core_estimator import get_performance_est as external_vc_estimator

from .network import get_allreduce_latency, get_alltoall_latency, get_allgather_latency, get_reduce_scatter_latency, init_network
...
tmpMemorization_allgather = {}
tmpMemorization_reducescatter = {}
...
def initialize_network():
    ...
    # init paths for network config and ip files for astra sim
    curr_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)))
...
def get_cycles(dataSize, num_devices, operation):
    ...
    if(operation == "AllReduce"):
        ...
        if key in tmpMemorization_allgather:
            # print(f"Cache hit for size={dataSize} and num_devices={num_devices}")
            return tmpMemorization_allgather[key]
        else:
            # print(f"Cache miss for size={dataSize} and num_devices={num_devices}")
            result = get_allgather_latency(list(range(num_devices)), dataSize)
            tmpMemorization_allgather[key] = result  # Store in the cache
            return result
    elif(operation == "ReduceScatter"):
        if key in tmpMemorization_reducescatter:
            # print(f"Cache hit for size={dataSize} and num_devices={num_devices}")
            return tmpMemorization_allgather[key]
        else:
            # print(f"Cache miss for size={dataSize} and num_devices={num_devices}")
            result = get_reduce_scatter_latency(list(range(num_devices)), dataSize)
            tmpMemorization_allgather[key] = result  # Store in the cache
            return result
...
def allreduce_estimator(phaze_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    ...
    # print(f"Allreduce with {num_devices} num_devices and tensorsize {tensorsize}")
...
def alltoall_estimator(phaze_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    from ..architecture import frequency
    start = time.time()
...
def gather_estimator(phaze_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    from ..architecture import frequency
    start = time.time()
    ...
    if(num_devices == 1):
        ...
        num_cycles = get_cycles(tensorsize, num_devices, "AllGather")
...
def scatter_estimator(phaze_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    from ..architecture import frequency
    start = time.time()
    ...
    if(num_devices == 1):
        ...
        num_cycles = get_cycles(tensorsize, num_devices, "ReduceScatter")
