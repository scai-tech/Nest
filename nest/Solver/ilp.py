...
from nest.Estimator import phaze_coretype_mapping, tc_configs, vc_configs
from nest.Estimator import create_core_config, get_flops
from nest.Estimator import convert_phaze_to_fused_graph, get_engine_type
    ...
    def generate_digraph_from_fused_graph(wgraph, mode="fwd"):
        ...
        for node in wgraph.nodes.values():
            ...
            if (node_attr["core_type"] == "Nop"):
                ...
                if "AllReduce" in node.node_desc or "AllToAll" in node.node_desc or "AllGather" in node.node_desc or "ReduceScatter" in node.node_desc:
                    e = e_s = latency_estimates["AR"][str(node.node_id)]
                    node_attr["comm"] = e[mode]['latency'] * 1000 # keep track of communication latency separately
                else:
...
def latency_per_layer(l_op_graph, acc_config, latency_estimates):
    ...
    # use ilp or sequential scheduling based on setting 
    if not configs.use_ilp:
        best_latency_fwd, energy_fwd, utilization_fwd = sequential__latency(fwd_graph, acc_config)
        best_latency_bwd, energy_bwd, utilization_bwd =  sequential__latency(bwd_graph, acc_config)
    else:
    ...
    utilization["TC"]["peak"] = 0
    utilization["TC"]["avg"] = 0
    utilization["VC"]["peak"] = 0
    utilization["VC"]["avg"] = 0
...
def sequential__latency(graph, acc_config):
    ...
    non_nop_nodes = [i for i in graph.nodes() if not graph.nodes[i]['node']['core_type'] == 'Nop']

    for j in non_nop_nodes:
        latency += graph.nodes[j]['node']['intra_op_latency']
        if graph.nodes[j]['node']['comm']:
            comm_lat += graph.nodes[j]['node']['comm']
    print("sequential: "+ str(latency/ 1000))
    print("communication: "+ str(comm_lat/ 1000))

    return (latency/ 1000, 0,0)
