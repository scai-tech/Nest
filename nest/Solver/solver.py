...
from nest.Estimator import get_configs_to_explore
from nest.Estimator import bandwidth, bytes_per_element, frequency

# from Estimator import bws, latency_per_level, devices_per_level
from nest.Estimator import configs
from nest.GraphExtractor import generate_out_filename
...
def write_explored_configs_to_file(model_name, micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree, sp_enabled, cp_degree):
    global explored_acc_configs
    module_type_str = "AR_" + \
        str(activation_recomputation) + "_HBM_" + \
        str(hbm_size) + "_phaze_solved"
    out_file = generate_out_filename(
        model_name, "json", micro_batch_size, max_tmp_width, sequence_length, module_type_str, ep_degree=ep_degree, sp_enabled=sp_enabled, cp_degree=cp_degree)

    out_dir = os.path.join(os.path.dirname(os.path.realpath(
        __file__)), f"output_{configs.setup_name}")
...
def run_solver(models_info, micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree, sp_enabled, cp_degree):
    ...
    while not converged:
        for model_name, model_info in models_info.items():
            ...
            out = solve(tmpc_models, latency_estimates, next_cc,
                        micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree,  sp_enabled, cp_degree)
    ...
    write_explored_configs_to_file(model_name, micro_batch_size, max_tmp_width,
                                   sequence_length, activation_recomputation, hbm_size/(1024*1024*1024), ep_degree, sp_enabled, cp_degree)

    return (best_cc, exec_strategies), estimation_time, ilp_time, dp_time


# executes per model
def solve(models, latency_estimates, cc, micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree, sp_enabled, cp_degree):
    ...
    return device_placement(model_name, layer_state, micro_batch_size, max_tmp_width, sequence_length, utilization, activation_recomputation, hbm_size, ep_degree, sp_enabled, cp_degree)


def device_placement(model_name, layer_state, micro_batch_size, max_tmp_width, sequence_length, utilization, activation_recomputation, hbm_size, ep_degree, sp_enabled, cp_degree):

    micro_batch_size = micro_batch_size * ep_degree
    num_micro_batches_in_batch = global_batch_size / micro_batch_size

    global dp_time

    # Load baseline configs for each requested comparison
    baselines = {}
    if configs.baseline_configs:
        for name in configs.baseline_configs:
            filepath = os.path.join(os.path.dirname(__file__), f"baselines_{configs.setup_name}", str(configs.num_accelerators), f"baseline_configs_{name}.py")
            if not os.path.exists(filepath):
                print(f"Baseline config file does not exist: {filepath}")
                continue
            spec = importlib.util.spec_from_file_location(f"baseline_configs_{name}", filepath)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            if model_name in module.baseline_config:
                baselines[name] = module.baseline_config[model_name]
                print(f"Loaded baseline config for {name}: ", baselines[name])

    dp_input = {"ep_degree": ep_degree,
                "sp_enabled": sp_enabled,
                "cp_degree": cp_degree,
                "maxMemoryPerDevice": hbm_size,
                "maxDevices": configs.num_accelerators,
                "bandwidth": bandwidth,
                "bws" : configs.bws,
    ...
    m_dir = os.path.join(os.path.dirname(os.path.realpath(
        __file__)), f"device_placement/inputs_{configs.setup_name}")
    ...
    out_file = generate_out_filename(
        model_name, "json", micro_batch_size, max_tmp_width, sequence_length, "out", ep_degree=ep_degree, sp_enabled=sp_enabled, cp_degree=cp_degree)
    ...
    status = subprocess.call(
        ["./nest/Solver/device_placement/device_placement", in_filepath, out_filepath])
    ...
    baseline_results = out.get("baselineResults", {})  # {} if not present
    thptfixed = {}
    for name, tpb in baseline_results.items():
        if tpb is not None:
            thptfixed[name] = global_batch_size / tpb
        else:
            thptfixed[name] = None  # or skip entirely
