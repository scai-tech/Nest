    ...
    def __init__(self,
        ...
        if useRay:
            ...
            for i in range(num_nodes):
                # Create a mock node dictionary that mimics the structure provided by Ray.
                # The crucial part is the "Resources" dictionary, which holds the
                # number of devices for this simulated node.
                mock_node_info = {
                    "Resources": {
                        # Assumes global_config.ray_accelerator_name is defined (e.g., "GPU")
                        global_config.ray_accelerator_name: float(num_devices_per_node)
                    },
                    # Add other mock keys for structural consistency if needed.
                    "NodeID": f"mock-node-{i}",
                    "Alive": True,
                    "NodeManagerAddress":  self.head_info['node_ip_address']
                }
                all_host_info.append(mock_node_info)

            # Create a list of device counts for each simulated node.
            # This logic can now directly mirror the Ray implementation.
            all_host_num_devices = []
            for host_info in all_host_info:
                number = host_info["Resources"][global_config.ray_accelerator_name]
                assert number.is_integer()
            ...
            all_host_ips = [self.head_info['node_ip_address']] * num_nodes
        ...
        if (num_devices_per_node or num_nodes) and useRay == True:
...
def init_global_cluster(cluster: str,
    ...
    if cluster == "local":
        ...
        global_cluster = DeviceCluster(num_nodes, num_devices_per_node, useRay=False)
        global_virtual_physical_mesh = (
            global_cluster.get_virtual_physical_mesh())
