...
# 100 Gbps = 12.5 GB/s
# 112.5 Gbps = 14.0625 GB/s
beta = [1/(12.5*10**9), 1/(112.5*10**9)]
alpha = [0.0003, 0.0001]
...
def training_dp(num_layers, num_devices, num_microbatches, submesh_choices,
    ...
    print(f"Best solution cost: {best_cost}") # NEST
...
def get_submesh_choices(
    ...
    if space == "all":
        ...
        i = 2
        while i <= num_hosts:
            if i * num_devices_per_host <= 128: ## NEST prevent large submesh sizes reduct compile time 
                submesh_choices.append((i, num_devices_per_host))
            i *= 2
        ...
        for mp_size in range(1, max_mp_dimension + 1):
            if num_devices % mp_size == 0:
                ...
                if batch_size % dp_size == 0:
                    if virtual_submesh.num_hosts <= 1:
                        mesh_beta = (beta[1], beta[1])  # logical mesh communication always within node, use faster bw
                        mesh_alpha = (alpha[1], alpha[1])
                    elif mp_size > virtual_submesh.num_devices_per_host:
                        mesh_beta = (beta[0], beta[0])  # logical mesh dim2 is larger than physical node, so we use the slower bandwidth for all communication
                        mesh_alpha = (alpha[0], alpha[0])
                    else:
                        mesh_beta = (beta[0], beta[1]) # logical mesh dim2 is less than physical node, so communication can use faster bw on this dim.
                        mesh_alpha = (alpha[0], alpha[1])
                    results.append((virtual_submesh.get_logical_mesh(
                        (dp_size, mp_size), mesh_alpha=mesh_alpha, mesh_beta=mesh_beta), {
                            "force_batch_dim_to_mesh_dim": 0
                        }))
        if num_devices > virtual_submesh.num_devices_per_host:
            mesh_alpha = (alpha[0], alpha[0])
            mesh_beta = (beta[0], beta[0])
        else:
            mesh_alpha = (alpha[1], alpha[1])
            mesh_beta = (beta[1], beta[1])
        results.append((virtual_submesh.get_logical_mesh((num_devices, 1), mesh_alpha=mesh_alpha, mesh_beta=mesh_beta), {}))
...
def cluster_layers_and_slice_mesh(
    ...
    if isinstance(stage_option, AutoStageOption):
        ...
        exit() ## NEST - we only care about the stage construction
