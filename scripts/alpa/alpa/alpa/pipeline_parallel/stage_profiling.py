...
from alpa.estimator.estimator import populate_estimates, HBM
...
class CompileWorker:
    ...
    def dummy_compile_stage_for_profiling(self, stage_id, config: CompileConfig,
                                    logical_mesh, autosharding_option,
                                    num_micro_batches):
        """
        Compile a single stage with auto sharding for estimation -- NEST.
        ...
        ## somewhere here we add the estimation tool chain to estimate runtime for HLO
        #
        # Add estimation tool chain for HLO
        #
        estimations = [(0.001, 0.001,0.1*GB,10*GB), (0.001,0.001,0.1*GB,10*GB)] # NEST, output 0 for everything for now [fwd, bwd].
            ...
            if module_name.endswith(APPLY_GRAD_MARKER_SUFFIX):
                ...
                try:
                    optimized_hlo = run_spmd_partitioner_pass(
                        ...
                        rewrite_for_grad_acc=rewrite_for_grad_acc,
                        rewrite_grad_acc_indices=acc_grad_outvars_indices)
                    print(f'compiled for stage:{stage_id} name: {module_name} module id: {module_id}')
                    # print(optimized_hlo.get_mhlo()) # NEST
                    # print(optimized_hlo.get_operators_and_dimensions())
                    estimates = populate_estimates(optimized_hlo.get_operators_and_dimensions(), logical_mesh)
                    # print(estimates) # NEST
                    estimations[module_id] = (estimates["AR"], estimates["Total_Latency"],  HBM*GB, HBM*GB)
                except IndexError as e:
                    logger.warning(f"Compilation error (spmd partitioner pass) "
                                   f"for stage {stage_id} : {e}")
                    return stage_id, None
        ...
        return stage_id, CompileOutput(acc_grad_module_compile_outputs,
                                       stage_plan,
                                       apply_grad_input_sharding_protos), estimations
    
    def dummy_compile_stage_for_profiling_v2(self, stage_id, stage_idx, config: CompileConfig,
                                    logical_mesh, autosharding_option,
                                    num_micro_batches):
        """
        Compile a single stage with auto sharding for estimation -- NEST.
        ...
        apply_grad_input_sharding_protos = None
        single_layer_cost = {
            # (submesh, module_id) : (comm_cost, compute_cost, peakMem, availMem)
            (0,0): (0.000, 64.552, 8.173*GB, 64*GB), # fwd
            (0,1): (0.000, 40.129, 9.299*GB, 64*GB), # bwd
            (1,0): (0.065, 71.331, 4.962*GB, 64*GB), # fwd
            (1,1): (0.123, 180.327, 5.087*GB, 64*GB), # bwd
            (2,0): (0.065, 48.276, 2.981*GB, 64*GB), # fwd
            (2,1): (0.124, 149.720, 2.981*GB, 64*GB), # bwd
            (3,0): (0.065, 25.076, 1.678*GB, 64*GB), # fwd
            (3,1): (0.124, 157.548, 1.922*GB, 64*GB), # bwd
            (4,0): (0.082, 14.180, 1.222*GB, 64*GB), # fwd
            (4,1): (0.154, 72.985, 1.389*GB, 64*GB), # bwd
            (5,0): (0.081, 9.004, 0.939*GB, 64*GB), # fwd
            (5,1): (0.155, 49.468, 1.202*GB, 64*GB), # bwd
            (6,0): (0.054, 7.551, 1.049*GB, 64*GB), # fwd
            (6,1): (0.347, 165.518, 1.670*GB, 64*GB), # bwd
            (7,0): (0.310, 11.303, 0.5*GB, 64*GB), # fwd
            (7,1): (0.393, 40.210, 0.6*GB, 64*GB), # bwd
        }
        print(stage_idx)
        start_layer, end_layer, submesh, autoconfig = stage_idx
        ...
        for module_id, module_name in enumerate(config.names):
            ...
            if module_name.endswith(APPLY_GRAD_MARKER_SUFFIX):
                ...
                key = (submesh, module_id)
                comm_cost, compute_cost, peakMem, availMem = single_layer_cost.get(key, (0.001, 0.001, 0.1*GB, 10*GB))
                estimations[module_id] = (comm_cost*num_layer, compute_cost*num_layer, peakMem*num_layer, availMem)
        ...
        return stage_id, None, estimations
...
class CompileWorkerPool(BaseWorkerPoolWrapper):
    ...
    def __init__(self, num_cpus, debug_mode=False):
        ...
        print(f"num of workers:{len(self.actors)}")
...
class HloCostModelProfileWorker:
    ...
    def profile(self, stage_id, compiled_module_output, stage_plan,
        ...
        # available_memory = self.prof_result.available_memory_per_device #NEST
        available_memory = 10*GB
...
def dummy_compile_all(stages, num_micro_batches, default_as_option, profile_results):
    """
    Compile all input stages.
    """
    num_cpus = int(
    ...
    for i, (stage_idx, stage_config, auto_sharding_config) in enumerate(stages):
        ...
        logical_mesh, autosharding_option_dict = auto_sharding_config
        compile_workers.submit(
            lambda w, v: w.dummy_compile_stage_for_profiling.remote(*v),
            (i, stage_config.compile_config, logical_mesh,
             dataclasses.replace(default_as_option, **
                                 autosharding_option_dict), num_micro_batches))
        num_compiled_stages += 1
    ...
    compiled_outputs = [None] * len(stages)
    estimations_outputs = [None] * len(stages) # NEST
    for _ in tqdm.tqdm(range(num_compiled_stages)):
        try:
            # here we not only compile (original), but also estimate the runtime - NEST
            i, compiled_output, estimations = compile_workers.get_next_unordered()
        except TimeoutError:
            logger.warning("Compile worker timeout")
            continue
        except RayActorError as e:
        ...
        compiled_outputs[i] = compiled_output
        estimations_outputs[i] = estimations # NEST
    ...
    compile_workers.shutdown()
    return compiled_outputs, estimations_outputs

def dummy_compile_all_v2(stages, num_micro_batches, default_as_option, profile_results):
    """
    Compile all input stages.
    """
    num_cpus = int(
    ...
    for i, (stage_idx, stage_config, auto_sharding_config) in enumerate(stages):
        ...
        logical_mesh, autosharding_option_dict = auto_sharding_config
        compile_workers.submit(
            lambda w, v: w.dummy_compile_stage_for_profiling_v2.remote(*v),
            (i, stage_idx, stage_config.compile_config, logical_mesh,
             dataclasses.replace(default_as_option, **
                                 autosharding_option_dict), num_micro_batches))
        num_compiled_stages += 1
...
def generate_module_profile_result_v2(raw_result: Tuple,
                                   profile_config: ModuleProfileConfig,
                                   compile_output: ModuleCompileOutput,
                                   logical_mesh_shape: Tuple[int, ...]):
    # NEST: note that compute_cost is total latency which is the comm_cost + actual computation cost 
    comm_cost, compute_costs, peak_memory, available_memory = raw_result
...
def dummy_distributed_profile_on_mesh(stages, meshes: Sequence[VirtualPhysicalMesh],
                                num_micro_batches, default_as_option,
                                auto_stage_option, profile_results):
    timers("stage-construction-compilation").start()
    ...
    print("- Compile all stages, get estimations")
    try:
        compiled_outputs, estimations_outputs = dummy_compile_all(stages, num_micro_batches,
                                       default_as_option, profile_results) # NEST - add estimations
    except RayActorError as e:
        logger.warning(f"Compilation fatal error: {e}")
        timers("stage-construction-compilation").stop()
        return profile_results
    ...
    # NEST
    print("- parse estimation results for all stages")
    # instead of profile results, we put the estimation ins profiled results format.
    # 
    # profile_results = dummy_profile_all(stages, compiled_outputs, meshes,
    #                     num_micro_batches, auto_stage_option,
    #                     profile_results) 
    profile_results = parse_estimation_results(stages, compiled_outputs, meshes,
                                  num_micro_batches, auto_stage_option,
                                  profile_results, estimations_outputs)
...
def parse_estimation_results(stages, compiled_outputs: Sequence[CompileOutput], meshes,
                num_micro_batches, auto_stage_option, profile_results, estimations_outputs): # NEST
    """Profile all compiled outputs on given meshes.
    ...
    # Precomputed estimates for peak mem for gpt3 175 B per layer per device for each device mesh
    gpt_peak_mem_estimates = { 
        0: (11*GB, 12*GB),
        1: (5.6*GB, 6.5*GB),
        2: (3.4*GB, 3.9*GB),
        3: (2.1*GB, 2.45*GB),
    ...
    for i, (compiled_output, stage) in enumerate(zip(compiled_outputs, stages)):
        ...
        for module_id, (acc_grad_module, profile_config) in enumerate(
            ...
            # get the HLO and previous estimates
            hlo = stage_compile_output.acc_grad_module_compile_outputs[module_id].hlo
            comm_lat, latency, dummy_peakMem, memCap = estimations_outputs[i][module_id]

            # try to estimate the memory requirements based on HLO
            try: 
                compiled = run_backend_compilation(
                    xb.get_backend(global_config.backend),
                    hlo,
                    stage_compile_output.stage_plan,
                    1,
                    bypass_device_assignment_check=True)

                peak_memory = compiled.total_allocation_size()
                comm_lat, latency, dummy_peakMem, memCap = estimations_outputs[i][module_id]
                estimations_outputs[i][module_id] = (comm_lat, latency, peak_memory, memCap)
                del compiled
            except:
                print(f"Using pre computed peak Mem for {stage_idx}, module {module_id}")
                first_layer, last_layer, mesh_id, _ = stage_idx
                num_layers = last_layer - first_layer + 1
                mem_fwd, mem_bwd = gpt_peak_mem_estimates[mesh_id] 
                if module_id == 0:
                    peak_memory = mem_fwd * num_layers
                else:
                    peak_memory = mem_bwd * num_layers
                comm_lat, latency, dummy_peakMem, memCap = estimations_outputs[i][module_id]
                estimations_outputs[i][module_id] = (comm_lat, latency, peak_memory, memCap)
            
            module_profile_result = generate_module_profile_result(
                estimations_outputs[i][module_id], stage_config.module_profile_configs[module_id],
                stage_compile_output.acc_grad_module_compile_outputs[module_id],
                stage_compile_output.stage_plan.logical_mesh_shape)
            print(f"result[{stage_idx}, {module_id}] "
                    f"= {module_profile_result}")
            profile_results[stage_idx].add_module_profile_result(
                module_id, module_profile_result)
...
def parse_estimation_results_v2(stages, compiled_outputs: Sequence[CompileOutput], meshes,
                num_micro_batches, auto_stage_option, profile_results, estimations_outputs): # NEST
    """Profile all compiled outputs on given meshes.
    ...
    for i, (compiled_output, stage) in enumerate(zip(compiled_outputs, stages)):
        ...
        for module_id in module_ids: 
            module_profile_result = generate_module_profile_result_v2(
            estimations_outputs[i][module_id], stage_config.module_profile_configs[module_id],
            None,
            None)
            print(f"result[{stage_idx}, {module_id}] "
...
def get_compute_cost(
    ...
    for mesh_id, submesh in reversed(list(enumerate(submesh_choices))):
        ...
        profile_results = dummy_distributed_profile_on_mesh(
