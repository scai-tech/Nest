...
from typing import Callable, Tuple, Union, Optional #NEST
...
class FusedScaleMaskSoftmax(nn.Module):
    ...
    def forward(self, input: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        # TODO: change for phaze, return the fixed function instead of the control path below
    ...
    def forward_fused_softmax(self, input, mask):
        ...
        if self.attn_mask_type == AttnMaskType.causal:
            # assert sq == sk, "causal mask is only for self attention" # NEST CP fix

            # input is 3D tensor (attn_batches, sq, sk)
            input = input.view(-1, sq, sk)
            probs = ScaledUpperTriangMaskedSoftmax.apply(input, scale)
            return torch.empty_like(input) #NEST CP fix
...
# class FusedScaleMaskSoftmax(nn.Module): #NEST from transformer_engine 
#     """
#     fused operation: scaling + mask + softmax
