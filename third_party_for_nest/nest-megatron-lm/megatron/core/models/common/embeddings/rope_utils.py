...
def _apply_rotary_pos_emb_thd(
    ...
    #TODO: Phaze: assume same sequence length, no need to iterate 
    # Assume all sequences in the batch have the same length
    seq_len = cu_seqlens[1] - cu_seqlens[0]  # Compute the uniform sequence length

    # No need for torch.split; process the entire tensor directly
    result = _apply_rotary_pos_emb_bshd(
        t.unsqueeze(1),  # Add a dimension if needed
        freqs,
        rotary_interleaved=rotary_interleaved,
        multi_latent_attention=multi_latent_attention,
        mscale=mscale,
