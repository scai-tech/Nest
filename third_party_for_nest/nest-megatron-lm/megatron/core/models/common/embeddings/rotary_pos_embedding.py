...
# TODO: Phaze: Wrap the arange function to prevent tracing

# TODO: torch fx arange function
def arange_for_fx(start, end, step, device, dtype):
    return torch.arange(start=start, end=end, step=step, device=device)
...
#     def forward(self, start=0, end=None, step=1, dtype=None, device=None):
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        #TODO: Phaze use custom arrange for tracing
        # self.inv_freq = 1.0 / (
        #     rotary_base ** (arange_for_fx(0, dim, 2, dtype=torch.float32, device=device) / dim)
        # )
        self.inv_freq = 1.0 / (
            rotary_base ** (arange_for_fx(0, dim, 2, dtype=torch.float32, device=device) / dim)
        )
    ...
    def get_freqs_non_repeated(self, max_seq_len: int, offset: int = 0) -> Tensor:
        ...
        #TODO: Phaze us custom arange for tracing
        seq = (
            arange_for_fx(0, max_seq_len, 1, dtype=torch.float32, device=self.inv_freq.device)
            + offset
        )
