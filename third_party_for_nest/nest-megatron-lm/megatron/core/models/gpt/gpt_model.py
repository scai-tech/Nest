...
class GPTModel(LanguageModule):
    ...
    def forward(
        ...
        if decoder_input is not None:
            ...
            # Context Parallelism: split position_ids, input_ids, attention_mask, AND labels BEFORE embedding #NEST
            if self.config.context_parallel_size > 1:
                from megatron.core import parallel_state
                cp_rank = parallel_state.get_context_parallel_rank()
                cp_size = self.config.context_parallel_size
                
                # Get sequence length from position_ids shape
                seq_len = position_ids.shape[1]  # position_ids is [b, s]
                local_seq_len = seq_len // cp_size
                
                start = cp_rank * local_seq_len
                end = (cp_rank + 1) * local_seq_len
                
                # Split all sequence-dimension tensors
                ...
                # Split attention mask along query dimension (dimension -2)
            ...
            if self.config.sequence_parallel:
                ...
                seq_len = labels.shape[1]  # labels is [b, s]
                local_seq_len = seq_len // tp_size
                
                start = tp_rank * local_seq_len
                end = (tp_rank + 1) * local_seq_len
