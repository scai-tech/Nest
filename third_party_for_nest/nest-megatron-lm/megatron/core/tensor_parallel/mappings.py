...
# TODO: change for phaze, added the wrapper function for torch.distributed.all_reduce
...
# TODO: change for phaze, all_reduce as a wrap for tracing
# torch.fx.wrap("_AllToAll")

# TODO: for phaze, to track the allreduce in backward pass
...
# TODO: for phaze, to track the allreduce in backward pass
torch.fx.wrap("gather_from_sequence_parallel_region")

# TODO: for phaze, to track the allreduce in backward pass
torch.fx.wrap("reduce_scatter_to_sequence_parallel_region")

# TODO: for phaze, to track the allreduce in backward pass
torch.fx.wrap("_split_along_first_dim")
...
def _split_along_first_dim(input_, group=None):
    ...
    group = get_tensor_model_parallel_group() ## NEST
    ...
    # TODO: change for phaze, we do not need the assert statement for tracing
    # assert (
    #     dim_size % world_size == 0
    # ), "First dimension of the tensor should be divisible by tensor parallel size"
    local_dim_size = dim_size // world_size
    rank = get_tensor_model_parallel_rank() ## NEST
...
def _gather_along_first_dim(input_, group=None, output_split_sizes=None, use_global_buffer=False):
    ...
    if output_split_sizes is None:
        ...
        output = output = gather_from_sequence_parallel_region(input_.contiguous(), world_size)
    else:
        ...
        output = output = gather_from_sequence_parallel_region(input_, world_size)
...
def _reduce_scatter_along_first_dim(
    ...
    group = get_tensor_model_parallel_group()
    # world_size = torch.distributed.get_world_size(group)
    world_size = get_tensor_model_parallel_world_size() ## NEST
    # Bypass the function if we are using only 1 GPU.
    if world_size == 1:
        return input_
