...
class Attention(MegatronModule, ABC):
    ...
    def _adjust_key_value_for_inference(
        ...
        #TODO: Phaze: inference_params = none
    ...
    def forward(
        ...
        # ===================================================
        # (Inference block omitted for brevity - same as before)
        if (
        ...
        if rotary_pos_emb is not None and not self.config.flash_decode:
            ...
            # [FIX FOR FX TRACING]
            # We unconditionally slice the RoPE embeddings to match the current Q/K lengths.
            # We CANNOT use 'if q_pos_emb.shape[0] != query.shape[0]' because query.shape is symbolic.
            
            # Slice q_pos_emb to [sq, 1, 1, D]
            q_pos_emb = q_pos_emb[:query.shape[0], :, :, :]
            
            # Slice k_pos_emb to [sk, 1, 1, D]
            k_pos_emb = k_pos_emb[:key.shape[0], :, :, :]
...
class SelfAttention(Attention):
    ...
    def get_query_key_value_tensors(self, hidden_states, key_value_states=None):
        ...
        t = torch.ones_like(hidden_states)
