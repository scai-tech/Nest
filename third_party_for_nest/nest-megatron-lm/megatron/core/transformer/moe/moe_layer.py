...
class ArangeForFx(torch.nn.Module):
    ...
    def forward(self, x, dtype=None, device=None):
...
class BaseMoELayer(MegatronModule, ABC):
    ...
    def __init__(self, config: TransformerConfig, layer_number: int = None):
        ...
        #TODO: Phaze
        self.arange_module = ArangeForFx()
    ...
    def __init__(
        ...
        if config.moe_token_dispatcher_type == "allgather":
            self.token_dispatcher = MoEAllGatherTokenDispatcher(
                self.num_local_experts, self.local_expert_indices, config=self.config, arange_module=self.arange_module
            )
