...
def ones_for_fx(size, device, dtype):
    return torch.ones(size, device=device, dtype=dtype)

# TODO: for phaze; adding allreduce in the wrap
torch.fx.wrap("ones_for_fx")

# TODO: torch fx arange function
def arange_for_fx(x, device):
    return torch.arange(x, device=device)
...
# TODO: torch fx arange function
def zeros_for_fx(size, device, dtype):
    return torch.zeros(size, device=device, dtype=dtype)

# TODO: for phaze; adding allreduce in the wrap
torch.fx.wrap("zeros_for_fx")
...
# TODO: Phaze we assign tokens equally to each expert
def balance_for_phaze(cost: torch.Tensor):
    num_tokens = cost.size(0)
    num_experts = cost.size(1)
    # Create a binary assignment matrix of shape (num_tokens, num_experts)
    assignments = torch.zeros_like(cost)
    
    # Number of tokens assigned to each pair of experts
    tokens_per_pair = num_tokens // (num_experts // 2)
    
    # Compute token indices
    token_indices = arange_for_fx(num_tokens,  device=torch.device('cuda'))
    pair_indices = (token_indices // tokens_per_pair) * 2
    
    # Ensure indices stay within bounds
    # pair_indices = torch.clamp(pair_indices, max=num_experts - 2)
    
    # Scatter assignments without a loop
    assignments.scatter_(1, pair_indices.unsqueeze(1), 1)
    assignments.scatter_(1, (pair_indices + 1).unsqueeze(1), 1)
...
def sinkhorn(cost: torch.Tensor, tol: float = 0.0001):
    ...
    cost = torch.exp(cost)
    # TODO: Phaze cost is a proxy object that we cannot directly get shape from 
    # d0 = torch.ones(cost.size(0), device=cost.device, dtype=cost.dtype)
    # d1 = torch.ones(cost.size(1), device=cost.device, dtype=cost.dtype)

    d0 = ones_for_fx(cost.size(0), device=torch.device('cuda'), dtype=torch.float32)
    d1 = ones_for_fx(cost.size(1), device=torch.device('cuda'), dtype=torch.float32)
    ...
    # TODO: Phaze cannot use loop for tracing 
    d0 = (1 / d0.size(0)) * 1 / (torch.sum(d1 * cost, 1) + eps)
    d1 = (1 / d1.size(0)) * 1 / (torch.sum(d0.unsqueeze(1) * cost, 0) + eps)
    error = torch.mean(torch.abs(d1_old - d1))
    d1_old = d1
...
def permute(
    ...
    if drop_and_pad and not (num_out_tokens is None):
        ...
        #TODO: Phaze use custom arange for tracing 
        token_indices = (
            arange_for_fx(num_tokens, device=routing_map.device).unsqueeze(0).expand(num_experts, -1)
        )
...
def unpermute(
    ...
    if probs is not None:
        ...
        if drop_and_pad:
            ...
            #TODO: use custom arange for tracing 
            indices_dim0 = arange_for_fx(num_experts, device=routing_map.device).unsqueeze(-1)
...
def sort_chunks_by_idxs(
    ...
    sorted_idxs = sorted_idxs.to(input.device)
    num_chunks = split_sizes.size(0)
    
    # Flatten everything except the first dimension
    orig_shape = input.shape
    input_flat = input.view(num_chunks, -1)
    
    # Index select on the simplified 2D tensor
    output_flat = torch.index_select(input_flat, 0, sorted_idxs)
    
    # Reshape back to the original multi-dim shape
