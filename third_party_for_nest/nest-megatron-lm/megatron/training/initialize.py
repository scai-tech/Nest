...
def _compile_dependencies():
    ...
    # TODO: change for phaze, torch.distributed cannot be used with a single gpu
    if args.rank == 0:
        start_time = time.time()
        print('> compiling dataset index builder ...')
        from megatron.core.datasets.utils import compile_helpers
    ...
    # TODO; change for phaze, cannot have torch.distributed on a single gpu
    # if torch.distributed.get_rank() == 0:
    if args.rank == 0:
        start_time = time.time()
        print('> compiling and loading fused kernels ...', flush=True)
        fused_kernels.load(args)
        # TODO: change for phaze, we do not need a barrier for the trace
...
def _initialize_distributed(get_embedding_ranks, get_position_embedding_ranks):
    ...
    # TODO: change for phaze, getting the world size from args
    ...
    if torch.distributed.is_initialized():
        ...
        # Call the init process
         # TODO: change for phaze; do not need to initialize the process group
